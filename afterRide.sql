...
BEGIN
    ...
    IF v_count = 0 THEN
    RAISE_APPLICATION_ERROR(-20010, 'Participante no inscrito en la versión');
    END IF;
END;
/
...
BEGIN
    BEGIN
        ...
        IF SYSDATE < v_inicio OR SYSDATE > v_fin THEN
            RAISE_APPLICATION_ERROR(-20012, 'Encuesta fuera de periodo');
        END IF;

    EXCEPTION
        WHEN NO_DATA_FOUND THEN
            NULL; -- deja que la FK maneje el error
    END;
END;
/
--- Soluciona algunos inserts que deberían funcionar.
UPDATE Encuestas
SET fechaInicio = SYSDATE - 1,
    fechaFin = SYSDATE + 30;
...
---------------- INDICES -------------------------
--------------------------------------------------
CREATE INDEX idx_registros_segmento
ON Registros(nombreSegmento);

CREATE INDEX idx_registros_participante
ON Registros(idParticipante);

CREATE INDEX idx_evaluaciones_encuesta
ON Evaluaciones(idEncuesta);

--------------------------------------------------
----------------- VISTAS -------------------------
--------------------------------------------------

CREATE OR REPLACE VIEW VW_SEGMENTOS_RAPIDOS AS
SELECT nombreSegmento, AVG(tiempo) AS tiempoPromedio
FROM Registros
GROUP BY nombreSegmento;

CREATE OR REPLACE VIEW VW_EVALUACIONES_ENCUESTA AS
SELECT idEncuesta, AVG(puntuacion) AS promedio
FROM Evaluaciones
GROUP BY idEncuesta;
...
----------------------------------------------------------------------------------------
----------------------------------------- Insertar DATOS de MBDA.data ------------------
----------------------------------------------------------------------------------------

INSERT INTO Participantes (id, idt, idn, pais, correo)
SELECT
    (SELECT MAX(id) FROM Participantes) + ROW_NUMBER() OVER (ORDER BY MIN(NUMERO)),
    'CC',
    TO_CHAR(MIN(NUMERO)),
    MIN(PAIS),
    correo
FROM mbda.DATA 
WHERE NUMERO IS NOT NULL AND CORREO IS NOT NULL
GROUP BY CORREO; 

INSERT INTO PERSONA (idParticipante, nombre)
SELECT
    p.id, MIN(d.nombre || ' ' || d.Apellido)
FROM Participantes p 
JOIN mbda.DATA d ON p.correo = d.correo
WHERE d.numero IS NOT NULL AND d.correo IS NOT NULL
GROUP BY p.id;
COMMIT;

INSERT INTO Ciclistas (idParticipante, nacimiento, categoria)
SELECT 
    p.id, NULL,
    CASE 
        WHEN MIN (d.categoria) > 5 THEN 5
        WHEN MIN (d.categoria) < 1 THEN 1
        ELSE MIN (d.categoria)
    END 
FROM Participantes p 
JOIN mbda.DATA d on p.correo = d.correo 
...
CREATE OR REPLACE PACKAGE BODY PC_REGISTROE AS
    ...
    BEGIN
        FOR r IN c_segmentos LOOP
            EXIT WHEN v_count >= 5;
            DBMS_OUTPUT.PUT_LINE('Segmentos: ' || r.nombreSegmento || ' - Tiempo promedio: ' || r.tiempoPromedio);
            v_count := v_count + 1;
        END LOOP;
...
CREATE OR REPLACE PACKAGE BODY PA_EVALUACIONES AS
    ...
    BEGIN
        ...
        EXCEPTION
            WHEN NO_DATA_FOUND THEN
                RAISE_APPLICATION_ERROR(-20050, 'Encuesta no existe');
        END;

        IF SYSDATE < v_inicio OR SYSDATE > v_fin THEN
            RAISE_APPLICATION_ERROR(-20041, 'Encuesta fuera de periodo');
        END IF;
        ...
        INSERT INTO Evaluaciones(
            ...
            comentarios, bienEspecifico,
            idEncuesta, referenciaAutor
        )
        VALUES (
            v_id, SYSDATE, p_puntuacion, 'Activo',
            p_comentarios, p_bienEspecifico,
            p_idEncuesta, p_referenciaAutor
        );
    END;
    ...
    BEGIN
        ...
        IF SQL%ROWCOUNT = 0 THEN
            RAISE_APPLICATION_ERROR(-20030, 'Evaluaciones no existe');
        END IF;
    END;
    ...
    BEGIN
        ...
        IF v_estado = 'Activo' THEN
            RAISE_APPLICATION_ERROR(-20031, 'Debe inactivar antes de eliminar');
        END IF;
    ...
    EXCEPTION
        WHEN NO_DATA_FOUND THEN
            RAISE_APPLICATION_ERROR(-20032, 'Evaluaciones no existe');
    END;
    ...
    BEGIN
        OPEN v_cursor FOR
            SELECT id, fecha, puntuacion, estado
            FROM Evaluaciones
            WHERE id = p_id;
        RETURN v_cursor;
    END;
    ...
    BEGIN
        OPEN v_cursor FOR
            SELECT id, puntuacion, comentarios, estado
            FROM Evaluaciones
            WHERE idEncuesta = p_idEncuesta;
        RETURN v_cursor;
    END;
...
END;
/
-- 3. Modificar revision y comentario de un Registros
BEGIN
    PC_REGISTROE.modificar(400, 2, 'Comentario actualizado');
END;
/
...
----------------- CRUDNoOK -------------------------
--------------------------------------------------
-- 1. Registrar un ciclista que no existe
BEGIN
    PC_REGISTROE.registrar(25, 1, 2, 'Error: ciclista inexistente', 'V1', 'S1', 9999);
END;
/
...
BEGIN
    PC_REGISTROE.registrar(
        20, 1, 3, 'Transaccion OK', 'V1', 'S1', 1
    );

    PC_REGISTROE.agregarFoto(
        300, 'Foto transaccion'
    );
...
GRANT EXECUTE ON PA_EVALUACIONES TO PA_MODERADOR;

-- SeguridadOk
-- 1. Persona consulta segmentos más rápidos 
BEGIN
    PA_PERSONAE.consultarSegmentosMasRapidos;
END;
/
-- 2. Persona consulta puntos de una carrera 
BEGIN
    PA_PERSONAE.consultarPuntosCarrera('C01');
END;
/
-- 3. Moderador consulta evaluaciones 
DECLARE
    c SYS_REFCURSOR;
BEGIN
    c := PA_EVALUACIONES.consultarEvaluaciones(1);
    CLOSE c;
END;
/
-- 4. Participante registra un nuevo registro 
BEGIN
    PA_PARTICIPANTEE.registrar(
        25, 1, 3, 'Registro desde seguridad', 'V1', 'S1', 1
    );
END;
/
-- 5. Participante agrega foto 
BEGIN
    PA_PARTICIPANTEE.agregarFoto(
        (SELECT MAX(numero) FROM Registros),
        'Foto seguridad OK'
    );
END;
/
--- SeguridadNoOK
-- 1. Persona intenta registrar (NO permitido)
BEGIN
    PA_PARTICIPANTEE.registrar(
        20, 1, 2, 'Intento indebido', 'V1', 'S1', 1
    );
END;
/
-- 2. Participante intenta consultar evaluaciones (NO permitido)
DECLARE
    c SYS_REFCURSOR;
BEGIN
    c := PA_EVALUACIONES.consultarEvaluaciones(1);
END;
/
-- 3. Persona intenta eliminar evaluaciones (NO permitido)
BEGIN
...
-- Paquetes de actores
DROP PACKAGE PA_PARTICIPANTEE;
DROP PACKAGE PA_PERSONAE;


-- Revocar permisos
REVOKE EXECUTE ON PA_PARTICIPANTEE FROM PA_PARTICIPANTE;
REVOKE EXECUTE ON PA_PERSONAE FROM PA_PERSONA;
REVOKE EXECUTE ON PA_EVALUACIONES FROM PA_MODERADOR;

-- Roles 
DROP ROLE PA_PARTICIPANTE;
DROP ROLE PA_PERSONA;
DROP ROLE PA_MODERADOR;
-----------------------------------
--------------- XCRUD -------------
-----------------------------------
...
DROP INDEX idx_registros_segmento;
DROP INDEX idx_registros_participante;
DROP INDEX idx_evaluaciones_encuesta;
DROP VIEW VW_SEGMENTOS_RAPIDOS;
DROP VIEW VW_EVALUACIONES_ENCUESTA;

--------------------------------------------------
------------ PRUEBA DE ACEPTACION ----------------
--------------------------------------------------

-- Historia: Un participante registra su desempeño en una carrera, consulta información del sistema y un moderador gestiona evaluaciones.


-- 1. Registrar un nuevo resultado 
BEGIN
    PA_PARTICIPANTEE.registrar(
        26, 1, 3, 'Buen tiempo en segmento', 'V1', 'S1', 1
    );
END;
/

-- 2 Confirmar consultando segmentos más rápidos
BEGIN
    PA_PERSONAE.consultarSegmentosMasRapidos;
END;
/

-- 3  Agregar evidencia del resultado
BEGIN
    PA_PARTICIPANTEE.agregarFoto(
        (SELECT MAX(numero) FROM Registros),
        'Foto evidencia del recorrido'
    );
END;
/

-- 4 Modificar comentario del registro
BEGIN
    PA_PARTICIPANTEE.modificar(
        (SELECT MAX(numero) FROM Registros),
        2,
        'Comentario ajustado tras revisión'
    );
END;
/

-- 5 Consultar puntos de una carrera
BEGIN
    PA_PERSONAE.consultarPuntosCarrera('C01');
END;
/

-- 6 Registrar evaluación válida 
BEGIN
    PA_EVALUACIONES.adicionar(
        5,
...
-- 7 Consultar evaluacioens de la encuesta
DECLARE
    c SYS_REFCURSOR;
BEGIN
    c := PA_EVALUACIONES.consultarEvaluaciones(1);
...
-- 8 Intento inválido puntuación fuera de rango
BEGIN
    PA_EVALUACIONES.adicionar(
        0,
        'Error puntuacion',
...
-- 9 Intento inválido participante inexistente
BEGIN
    PA_PARTICIPANTEE.registrar(
        20, 1, 2, 'Error participante', 'V1', 'S1', 9999
    );
END;
/

-- 10 Confirmación final consulta de evaluaciones
BEGIN
